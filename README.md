...
![Bad Banking Database Diagram](bad_banking_db.png)
...
![Good Banking Database Diagram](good_banking_db.png)
...
1. **Step 1: Setup Bad Design** Run `01_bad_design_setup.sql` to initialize the problematic database and populate it with duplicated sample data.
   
2. **Step 2: Test Inconsistency** Run `02_bad_design_workflow_test.sql`. This simulates a customer updating their address in their account profile only. The `UNION ALL` query query will expose the resulting data discrepancy.

3. **Step 3: Setup Good Design** Run `03_good_design_setup.sql` to deploy the normalized, centralized structure utilizing relational integrity.

4. **Step 4: Test Single Source of Truth** Run `04_good_design_workflow_test.sql`. Update the customer address once, and observe how `JOIN` queries and `VIEW` structures instantly reflect the correct, unified data across all banking profiles.
